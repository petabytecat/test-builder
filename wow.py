...
if len(sys.argv) < 2:
    print("Usage: python script.py <source_url> [include_subsection] [destination_file] [question_filter_json]")
    sys.exit(1)
...
# Optional 4th argument: JSON list of question codes
question_filter = None
if len(sys.argv) > 4:
    try:
        question_filter = json.loads(sys.argv[4])
        if not isinstance(question_filter, list):
            raise ValueError
        question_filter = [q.strip() for q in question_filter]
        print(f"Filtering for {len(question_filter)} question(s): {question_filter}")
    except Exception:
        print("Invalid question filter format. Must be JSON list, e.g. '[\"SPM.1.SL.TZ0.4\"]'")
        sys.exit(1)
...
# Parse question filters:
# - If more than 4 argv items, treat argv[4:] as a list of filters (handles unquoted filters)
# - If only argv[4] present, try to parse JSON; if JSON fails, treat it as comma-separated
question_filter = None
if len(sys.argv) > 4:
    if len(sys.argv) > 5:
        # multiple unquoted filters provided, e.g. ... dest SPM.1.SL.TZ0.4 SPM.1.SL.TZ0.9
        ...
        try:
            ...
            # fallback: allow comma separated without JSON
            question_filter = [s.strip() for s in raw.split(',') if s.strip()]
...
# Apply substring filter to links_dict (so all later logic sees only matched codes)
if question_filter:
    def matches_filter(code):
        return any(filt in code for filt in question_filter)
    before_count = len(links_dict)
    links_dict = {k: v for k, v in links_dict.items() if matches_filter(k)}
    after_count = len(links_dict)
    print(f"links_dict filtered: {before_count} -> {after_count} entries")

# Recompute sorted_questions from the (possibly filtered) links_dict
sorted_questions = sorted(links_dict.keys(), key=parse_question_code)
